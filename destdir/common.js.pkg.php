/*       Source:  Local Cache                                                                 */
/*     Location:  rsrc:109886:/html/js/common.js.pkg.php:60                                   */
/*      Machine:  10.16.139.109                                                               */
/*    Generated:  July 21st 2008 8:52:40 PM PDT                                               */
...
return function(){var _obj=obj||(this==window?false:this);var _args=args.slice();for(var jj=0;jj<arguments.length;jj++){_args.push(arguments[jj]);}
...
this.path&&(r+=this.path);q&&(r+='?'+q);this.fragment&&(r+='#'+this.fragment);return r;},isFacebookURI:function(){return!this.domain||!!this.domain.match(/(^|\.)facebook\.com$/i);},getUnqualifiedURI:function(){return new URI(this).setProtocol(null).setDomain(null).setPort(null);},getQualifiedURI:function(){var current=URI();var uri=new URI(this);if(!uri.getDomain()){uri.setProtocol(current.getProtocol()).setDomain(current.getDomain()).setPort(current.getPort());}
...
if(!written&&Util.fallbackErrorHandler){Util.fallbackErrorHandler(msg);}}else{if(type=='error'){msg+='\n\n'+Util.stack();(typeof(window['Env'])!='undefined')&&(Env.rlog)&&(typeof(window['debug_rlog'])=='function')&&debug_rlog(msg);}}},stack:function(){try{try{({}).llama();}catch(e){if(e.stack){var stack=[];var trace=[];var regex=/^([^@]+)@(.+)$/mg;var line=regex.exec(e.stack);do{stack.push([line[1],line[2]]);}while(line=regex.exec());for(var i=0;i<stack.length;i++){trace.push('#'+i+' '+stack[i][0]+' @ '+(stack[i+1]?stack[i+1][1]:'?'));}
...
this.uri=uri;return this;},getURI:function(){return this.uri;},setInitialHandler:function(fn){this.initialHandler=fn;return this;},setHandler:function(fn){if(typeof(fn)!='function'){Util.error('AsyncRequest response handlers must be functions. Pass a function, '+'or use bind() to build one.');}else{this.handler=fn;}
...
function dev_approval_popup(){(new pop_dialog()).show_choice("Developer Mode",{summary:'',body:"We noticed that you are the owner of a Facebook application.  When you have this mode enabled in the Translations application, you will have the ability to approve and un-approve translations done by your users.  Strings that are approved (either by you or by our automatic voting algorithm) will be available to be published live within 24 hours."},"Okay",function(){generic_dialog.get_dialog(this).hide()});}
...
track_moveable.prototype._track_drop=function(e){document.onmouseout=document.onmouseup=document.onmousemove=document.onclick=null;LinkController.assignHandler(this.link,function(e){return true;});if(!this.moved){new AsyncSignal('/ajax/navt.php',{app_id:this.app_id}).send();this.moved=false;}}
...
moveable_app.prototype._move=function(e){this.listContainer.top=mouseY(e)-this.mouseOffset;var oldParent=this.listContainer.parentNode;if(this.listContainer.nextNodeY&&this.listContainer.top>this.listContainer.nextNodeY){if(this.listContainer.nextList==null){var newParent=oldParent.nextSibling;newParent.appendChild(this.listContainer);oldParent.style.height=null;oldParent.appendChild(newParent.firstChild);}else{if(this.newList){_expand_more_list();var newParent=document.createElement('div');newParent.className='list_item';this.listContainer.nextList.insertBefore(newParent,this.listContainer.nextList.firstChild);newParent.appendChild(this.listContainer);oldParent.parentNode.removeChild(oldParent);}}}
...
if((_is_first_child(this.listContainer.parentNode,'app_list')&&this.listContainer.top<elementY(this.listContainer.parentNode))||(_is_last_child(this.listContainer.parentNode,'app_non_nav_list')&&this.listContainer.top>elementY(this.listContainer.parentNode))){this.listContainer.style.top=(elementY(this.listContainer.parentNode)+1)+'px';}else{this.listContainer.style.top=this.listContainer.top+'px';}
return false;}
function _is_first_child(elem,parent){return(elem.parentNode.id==parent)&&(elem.parentNode.firstChild==elem);}
function _is_last_child(elem,parent){return(elem.parentNode.id==parent)&&(elem.parentNode.lastChild==elem);}
function onload_side_nav_check(){var more=ge('more_link');if(more){more._hover_timeout=null;more.onmouseover=function(){this._hover_timeout=window.setTimeout(_expand_more_list,500);}.bind(more);more.onmouseout=function(){clearTimeout(this._hover_timeout);}.bind(more);more.onclick=toggle_more_list;}
_enforce_app_list_limits_and_save(false,'onload_side_nav');}
function _enforce_app_list_limits_and_save(force_save,context){var app_list_node=ge('app_list');var more_apps_node=ge('app_non_nav_list');var max_reached=false;var extra_pixel_amount=0;var rearrange_message=ge('rearrange_message');if(rearrange_message){extra_pixel_amount=rearrange_message.offsetHeight+6;}
...
moveable_app.prototype._drop=function(e){remove_css_class_name(this.listContainer,'floating_container');this.listContainer.style.top=null;this.listContainer.parentNode.style.height=null;_enforce_app_list_limits_and_save(true,'rearrange_order');if(this.listContainer.parentNode.parentNode.id!='app_non_nav_list'&&this.justOpened){window.setTimeout(_close_more_list,500);}
document.onmouseout=document.onmouseup=document.onmousemove=document.onclick=null;if(this.link){LinkController.assignHandler(this.link,function(e){return false;});}
return false;}
function toggle_more_list(){var list_expander=ge('expandable_more');if(!list_expander){return false;}
if(list_expander.style.display=='none'){_expand_more_list();}else{_close_more_list();}
return false;}
function _close_more_list(){var list_expander=ge('expandable_more');if(list_expander){list_expander.style.display='none';removeEventBase(document,'click',list_expander.offclick,list_expander.id);}
...
function _expand_more_list(){onunloadRegister(_close_more_list);var list_expander=ge('expandable_more');var more_link=ge('more_section');if(more_link){remove_css_class_name(more_link,'highlight_more_link');}
if(list_expander){list_expander.style.display='block';list_expander.offclick=function(e){if(!is_descendent(event_get_target(e),'sidebar_content')){_close_more_list();}}.bind(list_expander);addEventBase(document,'click',list_expander.offclick,list_expander.id);}
...
var RecaptchaOptions;var RecaptchaDefaultOptions={tabindex:0,callback:null,lang:'en'};var Recaptcha={widget:null,timer_id:-1,fail_timer_id:-1,type:'image',ajax_verify_cb:null,$:function(id){if(typeof(id)=="string"){return document.getElementById(id);}
...
return protocol+"//"+server;},_call_challenge:function(public_key){Recaptcha.fail_timer_id=setTimeout(Recaptcha.fail_timer_id==-1?"log_captcha_timeout(); create_captcha();":"create_captcha();",15000);var scriptURL=Recaptcha._get_api_server()+"/challenge?k="+public_key+"&ajax=1&cachestop="+Math.random();Recaptcha._add_script(scriptURL);},_add_script:function(scriptURL){Bootloader.loadResource({type:'js-ext',src:scriptURL});},_init_options:function(opts){var comb_opt=RecaptchaDefaultOptions;var user_opts=opts||{};for(var p in user_opts){comb_opt[p]=user_opts[p];}
...
function invite_preview(source_id,preview_id,toggle_ids){this.source=$(source_id);this.preview=$(preview_id);this.toggle_ids=(toggle_ids&&toggle_ids.map($))||[];this.refresh_preview=function(){var text=this.source.value;this.preview.innerHTML=htmlize(text);this.toggle_ids.map(text?show:hide);}.bind(this);return this;}
...
function Dialog(){Dialog._setup();this._pd=new pop_dialog();this._pd._dialog_object=this;this._show_loading=true;}
...
this._modal=modal;return this;},setContentWidth:function(width){this._content_width=width;this._dirty();return this;},setClassName:Dialog._basicMutator('_class_name'),setCloseHandler:function(close_handler){this._close_handler=call_or_eval.bind(null,this,close_handler);return this;},setFooter:Dialog._basicMutator('_footer'),setAsync:function(async_request){var handler=function(response){if(this._async_request!=async_request){return;}
...
function fbPresence(user,name,firstName,serverTime,inPopoutWindow,sitevars,popoutURL,loginURL){this.user=user;this.name=name;this.firstName=firstName;this.sitevars=sitevars;this.popoutURL=popoutURL;this.loginURL=loginURL;var viewportVector=Vector2.getViewportDimensions();this.maxTabOffset=46;this.maxTabHeight=viewportVector.y-this.maxTabOffset;this.updateServerTime(serverTime);this.pageLoadTime=this.getTime();this._init(inPopoutWindow);}
...
cached||this.appHideCache.push(app_id);},showHideDialog:function(clickedElement,app_id){var hide_handler=function(button_name){this.hideNotifications(app_id);setTimeout(presence.allowAutoClose.bind(presence),1100);var params={'app_id':app_id,'collapse':1};if('report'==button_name){params['spam']=1;}
new AsyncRequest().setURI('/ajax/notifications.php').setData(params).send();(typeof(toggleType)=='function')&&(typeof(hidden)!='undefined')&&(!hidden[app_id])&&toggleType(app_id,true,null,false,true,presence.user,false);};var canceler={};copy_properties(canceler,Dialog.CANCEL);canceler.handler=function(){presence.allowAutoClose();};presence.disallowAutoClose();var app_name=this.app_names[app_id];var dialog_handle=new Dialog().setTitle(tx('notif:hide-title',{'application-name':app_name})).setBody(tx('notif:hide-desc-short')).setButtons([{name:'hide',label:tx('notif:hide-button'),handler:bind(this,hide_handler,'hide')},{name:'report',label:tx('notif:report-button'),handler:bind(this,hide_handler,'report')},canceler]).setModal().show();return false;}};
...
if(to!=this.id){if(this.chatDisplay.gatedFeatures.sound&&this.chatDisplay.isSoundWindow&&statusControl.getSetting('sound')){var sound_player=document['so_sound_player']||window['so_sound_player'];if(sound_player){if(!sound_player.playSound&&sound_player.length){sound_player=sound_player[0];}
if(sound_player.playSound){sound_player.playSound(env_get('static_base')+'sound/pop.mp3');}}}
