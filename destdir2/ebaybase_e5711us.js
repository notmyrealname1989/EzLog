...
_encodeWrapperLoaded=true;var isUTF8=(typeof _GlobalNavHeaderUtf8Encoding!='undefined')?_GlobalNavHeaderUtf8Encoding:false;var vjoEscape=window.escape;var vjoUnescape=window.unescape;window.unescape=function(str){return decodeURIComponent(str);};var vjoDecodeURI=window.decodeURI;window.decodeURI=function(str){try{return vjoDecodeURI(str);}catch(e){return vjoUnescape(str);}};var vjoDecodeURIComponent=window.decodeURIComponent;window.decodeURIComponent=function(str){try{return vjoDecodeURIComponent(str);}catch(e){return vjoUnescape(str);}};var vjoEncodeURIComponent=window.encodeURIComponent;_ebayv4enc.encodeURIComponent=vjoEncodeURIComponent;window.encodeURIComponent=function(str){var utf8=isUTF8;if(utf8){return vjoEncodeURIComponent(str);}
return vjoEscape(str);};var vjoEncodeURI=window.encodeURI;window.encodeURI=function(str){var utf8=isUTF8;if(utf8){return vjoEncodeURI(str);}
...
else{return false;}};this.init=function(){if(this.isTop()){this.track();}};this.track=function(){var clientTime=new Date().getTime();var offset=this.getClientTimeOffset(clientTime);var tpim=this.getTpimCookielet(offset,clientTime);if(typeof(RoverNsCapable)=='undefined'&&(tpim==null||tpim=="")){var oD=this.oDocument,d=oD.doc,r=d.referrer;if(!(r==null||r==""||r.match(this.ebaysites))){this.writeImg();}}};this.getTpimCookielet=function(cos,clientTime){var oCJ=ebay.oDocument.oCookieJar;var npiiCookieVal=oCJ.readCookie("npii");var tpimval=npiiCookieVal.replace(new RegExp("(.*\\^tpim\/)([a-fA-F0-9]+)\\^.*"),"$2");var tpimexp=tpimval.substring(tpimval.length-8);var expTime=tpimexp.hex2Dec()*1000+cos*1000;var gmtDelta=expTime-clientTime;var expiredTime=15552000000;if(gmtDelta>expiredTime||gmtDelta<0){oCJ.writeCookielet("npii","tpim","");return"";}
return oCJ.readCookielet("npii","tpim");};this.addRotationId=function(pKey,pValue)
{this.aRotationIDs[pKey]=pValue;};this.writeImg=function()
